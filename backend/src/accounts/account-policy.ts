...
export function canUseAsDebitSourceForBanking(a: AccountPolicyFields): { ok: boolean; reason?: string } {
  ...
  if (!isDeposit(a) && a.type !== AccountType.CREDIT) return { ok: false, reason: 'unsupported_type' };
  if (a.type === AccountType.CREDIT && a.cardLifecycle !== CardLifecycleStatus.ACTIVE) {
    return { ok: false, reason: 'card_inactive' };
  }
